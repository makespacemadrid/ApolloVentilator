...
// Set output range
void PIDAutotuner::setOutputRange(float min_, float max_) {
  minOutput = min_;
  maxOutput = max_;
}
...
float PIDAutotuner::tunePID(float input) {
  ...
  if (!output && input < targetInputValue) {
    ...
    float ku = (4.0 * ((maxOutput - minOutput) / 2.0)) / (M_PI * (_max - _min) / 2.0);
